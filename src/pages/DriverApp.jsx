        ...
        {activeTrip ? (
          <>
            <div style={{ textAlign: 'center', padding: '10px 0' }}>
              <div style={{ fontSize: 12, color: '#3FA9F5', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>VIAJE ACTIVO</div>
              <h2 style={{ color: 'white', fontSize: 24, margin: 0 }}>ID #{activeTrip.id.substring(0,4).toUpperCase()}</h2>
            </div>

            {/* TRIP CARD */}
            <div style={{ background: '#1A1F26', borderRadius: 16, border: '1px solid #3FA9F5', boxShadow: '0 8px 30px rgba(63, 169, 245, 0.15)', overflow: 'hidden' }}>
              <div style={{ background: 'rgba(63, 169, 245, 0.1)', padding: '16px 20px', borderBottom: '1px solid rgba(63, 169, 245, 0.2)', display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: '#E5E7EB', fontWeight: 600 }}>Estado Actual:</span>
                <span style={{ color: '#3FA9F5', fontWeight: 800 }}>{activeTrip.estado}</span>
              </div>
                ...
                <div style={{ marginTop: 24, paddingTop: 16, borderTop: '1px dashed #2A2F36', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  ...
                  <div>
                    ...
                    <div style={{ color: '#4ADE80', fontWeight: 600 }}>
                      {activeTrip.quien_cobro === 'Chofer' ? `$${activeTrip.precio_estimado} Efectivo` : 'NO COBRAR (Cta.Cte.)'}
                    </div>
                  </div>
                </div>
            ...
            {/* ACTION BUTTONS */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 12, marginTop: 'auto' }}>
               {activeTrip.estado === 'Pendiente' && (
                 <button onClick={() => handleUpdateStatus(activeTrip.id, 'En Curso')} style={{ background: '#3b82f6', color: 'white', border: 'none', padding: 18, borderRadius: 12, fontSize: 16, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                    <Navigation size={20}/> Me dirijo al Origen
                 </button>
               )}
               {activeTrip.estado === 'En Curso' && (
                 <button onClick={() => handleUpdateStatus(activeTrip.id, 'Pasajero a Bordo')} style={{ background: '#F59E0B', color: 'white', border: 'none', padding: 18, borderRadius: 12, fontSize: 16, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                    🚙 Pasajero a Bordo
                 </button>
               )}
               {(activeTrip.estado === 'Pasajero a Bordo' || activeTrip.estado === 'En Curso') && (
                 <button onClick={() => handleUpdateStatus(activeTrip.id, 'Finalizado')} style={{ background: '#10B981', color: 'white', border: 'none', padding: 18, borderRadius: 12, fontSize: 16, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                    <CheckCircle2 size={24}/> Finalizar Viaje
                 </button>
               )}
            </div>
          </>
